...
启用后，Nine1Bot 会直接提供内置的 `browser_*` 工具。
不需要也不应该再在 `mcp` 里额外配置 `browser-mcp-server`，否则会出现重复的 `browser_browser_*` 工具入口。

> **注意**：浏览器扩展使用敏感权限（debugger、scripting、all_urls）来实现完整的浏览器自动化。请仅在可信环境中使用。

**旧配置迁移：**

- 旧的 `mcp.browser` + `browser-mcp-server` 配置已废弃，启动时会被自动忽略，并提示改用内置 `browser_*`
- 旧的 `BRIDGE_URL=http://127.0.0.1:18793` 已废弃，浏览器能力现在通过内置 `browser_*` 工具提供
- 旧的 `browser.bridgePort` 已废弃并会被忽略；保留 `"browser": { "enabled": true }` 即可
